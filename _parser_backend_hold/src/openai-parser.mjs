...
function chunkText(text, chunkSize = MAX_PDF_CHUNK_CHARS, maxChunks = Number.POSITIVE_INFINITY) {
  const chunks = [];
  let index = 0;
...
function buildTargetTokens(targetDancerNames = []) {
  return [...new Set(
    targetDancerNames
...
function selectRelevantPdfChunks(pdfChunks, targetDancerNames = []) {
  if (!pdfChunks.length) {
    return [];
  }
...
export async function parseCompetition(body, options = {}) {
