...
export default function ScheduleScreen() {
  ...
  const orderedTimelineEvents = overview
    ? [...overview.timelineEvents].sort(
        (left, right) => new Date(left.datetime).getTime() - new Date(right.datetime).getTime()
      )
    : [];
  const competitionMomentCount = orderedTimelineEvents.filter((event) => event.type === "competition").length;
  const competitionMoments = orderedTimelineEvents.filter((event) => event.type === "competition").slice(0, 3);
  ...
  return (
    ...
    >
      <YStack gap="$2">
        ...
        <Text fontSize={36} fontWeight="800" color="$color" letterSpacing={-1.1} lineHeight={40}>
        ...
        <XStack gap="$2" alignItems="center" flexWrap="wrap">
          ...
          <Text fontSize={16} color="$secondaryText">
            {visibleEntries.length} routines · {displayDancers.length} dancer{displayDancers.length === 1 ? "" : "s"}
            {competitionMomentCount ? ` · ${competitionMomentCount} key event${competitionMomentCount === 1 ? "" : "s"}` : ""}
            {` · first at ${overview?.nextEntry?.eventTime ?? "TBD"}`}
          </Text>
        </XStack>
      ...
      {competitions.length > 1 ? (
        <SectionCard
          title="Saved weekends"
          subtitle="Switch competitions here when you want the overview to follow a different event."
        >
      ...
      {competitionMoments.length ? (
        <SectionCard
          title="Competition-wide moments"
          subtitle="Awards, judges breaks, and other parent-important timing cues that affect the whole day."
        >
          <YStack gap="$3">
            {competitionMoments.map((event) => (
      ...
      <SectionCard
        title="Day flow"
        subtitle={`${overview?.dayLabel ?? "Today"} in time order so you can scan once and trust what comes next.`}
      >
        <YStack gap="$3">
          {orderedTimelineEvents.slice(0, 5).map((event) => (
            <TimelineEventCard key={event.id} event={event} />
          ))}
          <Button
            alignSelf="flex-start"
          ...
          <YStack gap="$3">
            <Pressable onPress={() => router.push(`/competition/${competition.id}/full-schedule`)}>
              ...
              <YStack gap="$1">
                ...
                <Text fontSize={14} color="$secondaryText" lineHeight={21}>
                  {tomorrowGroup.entries.filter((entry) => entry.itemType === "routine").length} routine{tomorrowGroup.entries.filter((entry) => entry.itemType === "routine").length === 1 ? "" : "s"} · first up at {tomorrowGroup.entries.find((entry) => entry.itemType === "routine")?.eventTime ?? "TBD"}
                </Text>
              </YStack>
            ...
            <XStack gap="$3">
              {tomorrowGroup.entries.filter((entry) => entry.itemType === "routine").slice(0, 2).map((entry) => (
