...
function mergeChunkRows({
  ...
  chunkResults: Array<{
    ...
    competitionEvents?: Array<Record<string, string | null>>;
  }>;
  targetDancerNames: string[];
}) {
  const rows = chunkResults.flatMap((result) => result.rows || []);
  const competitionEvents = chunkResults.flatMap((result) => result.competitionEvents || []);
  ...
  const normalizedCompetitionEvents = competitionEvents
    ...
    .map((event, index) => ({
      ...
      locationLabel: event.locationLabel ? String(event.locationLabel) : null,
    }))
  ...
  return {
    ...
    competitionEvents: normalizedCompetitionEvents,
    selectedDancers
  };
}
...
function buildChunkSchema() {
  return {
    ...
    schema: {
      ...
      required: ["rows", "competitionEvents"],
      properties: {
        rows: {
          type: "array",
          items: {
          ...
          content: [
            {
              ...
              text: [
                ...
                "Return dancer schedule rows plus competition-wide events.",
                "Keep competition-wide items like awards, judges breaks, meal breaks, and ballroom resets in competitionEvents.",
                "Exclude headers, banners, overall division labels, studio ads, and blank lines.",
