...
export const parsingService = {
  ...
  async parseCompetitionFromPdf(asset: StoredScheduleAsset): Promise<ParsingResult> {
    ...
    if (hasRemoteParser) {
      return {
        payload: await parserApiService.parseUploadedAsset("pdf", asset),
        metadata: {
          sourceType: "pdf",
          isMock: false,
          parserMode: "remote-ai",
  ...
  async parseCompetitionFromScreenshot(asset: StoredScheduleAsset): Promise<ParsingResult> {
    // OpenClaw can take over orchestration here later without changing screen code.
    if (openClawService.isConfigured()) {
      return {
        payload: await openClawService.parseCompetition({
    ...
    if (hasRemoteParser) {
      return {
        payload: await parserApiService.parseUploadedAsset("screenshot", asset),
        metadata: {
          sourceType: "screenshot",
          isMock: false,
          parserMode: "remote-ai",
...
,
  ...
  async startCompetitionParseFromLink(link: LinkImportInput, targetDancerNames?: string[]): Promise<ParsingJob | ParsingResult> {
    if (hasRemoteParser) {
      return parserApiService.createParsingJob({
        sourceType: "link",
        link: {
          ...link,
          targetDancerNames: targetDancerNames ?? link.targetDancerNames
        },
        targetDancerNames: targetDancerNames ?? link.targetDancerNames
      });
    }
