...
export const storageService = {
  ...
  async uploadImportedAsset(asset: ImportedScheduleAsset): Promise<StoredScheduleAsset> {
    ...
    if (hasSupabaseStorageConfig && supabase) {
      ...
      const signedUrlResult = await supabase.storage.from(bucket).createSignedUrl(objectPath, signedUrlTtlSeconds);

      if (signedUrlResult.error || !signedUrlResult.data?.signedUrl) {
        const error = signedUrlResult.error as SupabaseStorageErrorLike | null;
        throw new Error(
          [error?.error || "supabase-storage/signed-url", error?.message, error?.statusCode]
            .filter(Boolean)
            .join(" :: ")
        );
      }
