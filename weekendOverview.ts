...
export function buildWeekendOverview(params: {
  ...
  const nextEntry = dayEntries.find((entry) => new Date(entry.datetimeStart) >= now) ?? dayEntries[0];
  ...
  return {
    ...
    summary: `${dayEntries.length} routines today${dayCompetitionEvents.length ? ` · ${dayCompetitionEvents.length} comp-wide event${dayCompetitionEvents.length === 1 ? "" : "s"}` : ""} · first at ${dayEntries[0].eventTime}`,
    nextEntry,
    nextEntryDancerName,
    prepStartTime,
    departureTime: lodgingPlan?.recommendedDepartureTime,
